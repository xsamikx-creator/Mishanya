(() => {
  const ADMIN_HASH='e51bedd3342a874f29eaf21360f5b730289ee03d89b7cc7b82f3b36f4a07988a';
  ...
  function init(){
    ...
    let badge=null;
    const ensureBadge=()=>{
      if(!access.getState().isAdmin){badge?.remove();badge=null;return;}
      if(!badge){badge=el('div',{class:'admin-badge',id:'adminModeBadge'},'👑 Администратор');document.body.appendChild(badge);badge.onclick=open;}
      const s=access.getState();badge.textContent=s.adminPreview?'👀 Режим Мушвига':'👑 Администратор';
    };
    const close=()=>backdrop.classList.remove('open');
    const open=()=>{const state=access.getState();if(!state.isAdmin)return;document.querySelector('#togglePreview').textContent=state.adminPreview?'👑 Вернуться в режим администратора':'👀 Предпросмотр как Мушвиг';backdrop.classList.add('open');};
    document.querySelector('#adminClose').onclick=close; backdrop.onclick=e=>{if(e.target===backdrop)close();};
    ...
    title?.addEventListener('click',async()=>{
      taps++;clearTimeout(timer);timer=setTimeout(()=>taps=0,2200);
      if(taps<7)return;
      taps=0;
      if(access.getState().isAdmin){open();return;}
      const code=prompt('Введите код администратора:');
      if(!code)return;
      if(await sha256(code.trim())===ADMIN_HASH){access.activateAdmin();ensureBadge();alertBox('👑 Этот iPhone зарегистрирован как администратор.');open();}
      else alertBox('Неверный код администратора.');
    });
    window.addEventListener('mushvig:access-change',ensureBadge);
    ensureBadge();
  }
